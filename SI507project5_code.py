...
#*** collaborated with Saul Hankin & Tyler Hoff on code ideation and work.
...
tumblr_result = get_data_from_api(tumblr_url,"Tumblr",tumblr_params)
...
for blogpost in tumblr_result['response']['posts']:
	if blogpost['type'] == "photo":
		strwrt1+= "\n"
		strwrt1+= blogpost['slug']
		strwrt1+= ","
		strwrt1+= blogpost['date']
		strwrt1+= ","
...
strwrt2 = ""
for blogpost in tumblr_result['response']['posts']:
	if blogpost['type'] == "answer":
		strwrt2+= "\n"
		strwrt2+= blogpost['slug']
		strwrt2+= ","
		strwrt2+= blogpost['date']
		strwrt2+= ","
		strwrt2+= str(blogpost['note_count'])
		strwrt2+= ","
		strwrt2+= blogpost['summary'].replace(',',"")
		strwrt2+= ","
		strwrt2+= blogpost['question'].replace(',',"")
		strwrt2+= ","
		strwrt2+= blogpost['asking_name'].replace(',',"")
		
		
print(len(tumblr_result['response']['posts']))

tumbler_csv1 = 'tumblr_photos.csv'
tumbler_csv2 = 'tumblr_answers.csv'

try:
    open(tumbler_csv1, 'r')
    fcsv = open(tumbler_csv1, 'a') #write a new line, do not write over existing data
    fcsv.write(strwrt1)
    fcsv.close()
except:
    columnheaders = ("title,date,note_count,Summary,Tag1, Tag2, Tag3")
    fcsv = open(tumbler_csv1, 'a') #write a new line, do not write over existing data
...
try:
    open(tumbler_csv2, 'r')
    fcsv2 = open(tumbler_csv2, 'a') #write a new line, do not write over existing data
    fcsv2.write(strwrt2)
    fcsv2.close()
except:
    columnheaders2 = ("title,date,note_count,summary,question,questioner")
    fcsv2 = open(tumbler_csv2, 'a') #write a new line, do not write over existing data
    fcsv2.write(columnheaders2)
    fcsv2.write(strwrt2)
    fcsv2.close()			
